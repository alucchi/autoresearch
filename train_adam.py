...
parser.add_argument("--optimizer", type=str, default="adam", choices=["muon", "adam", "sgd"],
...
# Optimization
TOTAL_BATCH_SIZE = 2**16 # ~65K tokens: 8x more steps per budget
MATRIX_LR = 3e-3         # LR for transformer blocks + lm_head
EMBED_LR = 0.4           # high LR for token/value embeddings
SCALAR_LR = 0.5          # original SCALAR_LR value for x0/resid
ADAM_BETAS = (0.80, 0.90)
ADAM_EPS = 1e-8
WEIGHT_DECAY = 0.1
WARMUP_RATIO = 0.05
WARMDOWN_RATIO = 0.4
...
# Model size (set by --model preset above)
DEVICE_BATCH_SIZE = _batch_size
...
# 3-group: high LR for embeddings, separate LR for scalars, standard for matrices
embed_params = list(model.transformer.wte.parameters()) + list(model.value_embeds.parameters())
scalar_params = [model.resid_lambdas, model.x0_lambdas]
skip_ids = {id(p) for p in embed_params + scalar_params}
other_params = [p for p in model.parameters() if id(p) not in skip_ids]
...
for group in optimizer.param_groups:
    group["initial_lr"] = group["lr"]
print(f"Optimizer: AdamW 3-group embed={EMBED_LR} scalar={SCALAR_LR} matrix={MATRIX_LR}")
